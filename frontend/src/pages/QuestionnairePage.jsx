...
export default function QuestionnairePage() {
  ...
  const handleIntakeSubmit = async (formData) => {
    ...
    setError('');
    try {
      const result = await registerStudent({ ...formData, planType, language });
    ...
    <div className="min-h-screen bg-slate-50">
      ...
      <div className="bg-primary-800 text-white px-4 py-4">
        <div className="max-w-2xl mx-auto flex items-center gap-3">
          ...
          <span className="font-semibold">{qp.header}</span>
          <span className={`ml-auto text-xs px-2.5 py-1 rounded-full font-medium ${planType === 'paid' ? 'bg-amber-500 text-white' : 'bg-white/20 text-white'}`}>
            {planType === 'paid' ? qp.premium : qp.free}
          </span>
        </div>
      </div>
      ...
      <div className="max-w-2xl mx-auto px-4 py-8 page-enter">
        ...
        {step === STEPS.LOADING_QUESTIONS && (
          <div className="flex flex-col items-center justify-center py-24 gap-4">
            ...
            <p className="text-slate-600 font-medium text-lg">{qp.loadingQuestions}</p>
          </div>
        )}
        ...
        {step === STEPS.SUBMITTING && (
          <div className="flex flex-col items-center justify-center py-24 gap-4">
            <LoadingSpinner size="xl" />
            <p className="text-slate-600 font-medium text-lg">{qp.generatingReport}</p>
          </div>
        )}
      </div>
    </div>
